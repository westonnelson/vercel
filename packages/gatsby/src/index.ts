...
/*
- SSR Pages: `page-ssr/routes/component---src-pages-dog-tsx.js`
- Functions: `functions/hello.js`
*/
...
export const build: BuildV2 = async ({
  ...
  if (meta.isDev) {
    throw new Error('Detected `@vercel/gatsby` dev but this is not supported');
  }
  ...
  if (buildCommand) {
    ...
    if (!versionRange || !validRange(versionRange)) {
      console.log(
        'WARNING: Unable to detect Gatsby version in package.json devDependencies'
      );
  ...
  for (const [funcName, fileFsRef] of Object.entries(functionFiles)) {
    ...
    for (const filePath of fileList.concat(esmFileList)) {
